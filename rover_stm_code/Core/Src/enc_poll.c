...
void timer_update_TX(){
	for (int i = 0; i < NUM_QUAD; i++){
		...
		TxData_buf[12*i] = dat[i]&0xFF000000>>24;
		TxData_buf[12*i+1] = (dat[i]&0x00FF0000)>>16;
		TxData_buf[12*i+2] = (dat[i]&0x0000FF00)>>8;
		TxData_buf[12*i+3] = (dat[i]&0x000000FF);

		TxData_buf[12*i+4] = dat[i+1]&0xFF000000>>24;
		TxData_buf[12*i+5] = (dat[i+1]&0x00FF0000)>>16;
		TxData_buf[12*i+6] = (dat[i+1]&0x0000FF00)>>8;
		TxData_buf[12*i+7] = (dat[i+1]&0x000000FF);

		TxData_buf[12*i+8] = dat[i+2]&0xFF000000>>24;
		TxData_buf[12*i+9] = (dat[i+2]&0x00FF0000)>>16;
		TxData_buf[12*i+10] = (dat[i+2]&0x0000FF00)>>8;
		TxData_buf[12*i+11] = (dat[i+2]&0x000000FF);
	}
}
...
void drill_update_TX(){
	...
	drill_dat[2] = gpio_diff2;

	TxData_buf[12*NUM_QUAD] = drill_dat[0]&0xFF000000>>24;
	TxData_buf[12*NUM_QUAD+1] = (drill_dat[0]&0x00FF0000)>>16;
	TxData_buf[12*NUM_QUAD+2] = (drill_dat[0]&0x0000FF00)>>8;
	TxData_buf[12*NUM_QUAD+3] = (drill_dat[0]&0x000000FF);

	TxData_buf[12*NUM_QUAD+4] = drill_dat[1]&0xFF000000>>24;
	TxData_buf[12*NUM_QUAD+5] = (drill_dat[1]&0x00FF0000)>>16;
	TxData_buf[12*NUM_QUAD+6] = (drill_dat[1]&0x0000FF00)>>8;
	TxData_buf[12*NUM_QUAD+7] = (drill_dat[1]&0x000000FF);

	TxData_buf[12*NUM_QUAD+8] = drill_dat[2]&0xFF000000>>24;
	TxData_buf[12*NUM_QUAD+9] = (drill_dat[2]&0x00FF0000)>>16;
	TxData_buf[12*NUM_QUAD+10] = (drill_dat[2]&0x0000FF00)>>8;
	TxData_buf[12*NUM_QUAD+11] = (drill_dat[2]&0x000000FF);
}
